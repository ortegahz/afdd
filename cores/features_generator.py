...
class FeaturesGeneratorXGB(FeaturesGeneratorBase):
    ...
    @staticmethod
    def _features_fft_generate(data, num_intervals=256):
    ...
    @staticmethod
    def _features_dummy_generate(data, num_intervals=8):
        data_array = np.array(data)
    ...
    def generate(self, x, y=None):
        ...
        _features, _feature_names = self._features_fft_generate(x)
        features_lst.append(_features)
        feature_names_lst.extend(_feature_names)
        _features, _feature_names = self._features_dummy_generate(x)
        features_lst.append(_features)
        feature_names_lst.extend(_feature_names)
